...
export const projects = {
  ...
  create: (title, description = "") =>
    request("/projects", { method: "POST", body: { title, description } }),
...
export const tasks = {
  list: ({ projectId, page = 1, perPage = 10, status = "all", sort = "due_date" }) =>
    request(
      `/tasks?project_id=${projectId}&page=${page}&per_page=${perPage}&status=${status}&sort=${sort}`
    ),
